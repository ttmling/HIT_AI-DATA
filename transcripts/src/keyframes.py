...
def extract_raw_frames(video_path: Path, frames_dir: Path, interval_seconds: int) -> list[Path]:
    """Cắt video thành các ảnh frame thô bằng FFMPEG."""
...
def extract_features(image_path: Path) -> torch.Tensor:
    ...
    with torch.no_grad(): # Tắt tính đạo hàm để tiết kiệm RAM/VRAM
        outputs = MODEL(**inputs)
        # Lấy vector đặc trưng ở tầng cuối cùng (đại diện cho toàn bộ nội dung ảnh)
        image_features = outputs.pooler_output
        # Chuẩn hóa vector về độ dài bằng 1 để tính toán độ tương đồng chính xác hơn
